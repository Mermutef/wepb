...
class FetchChairmanDirectionID (
    private val selectChairmanByDirectionByID: (Int) -> User?,
) : (Int) -> Result4k<User, DirectionFetchingError> {

    override operator fun invoke(directionId: Int): Result4k<User, DirectionFetchingError> =
        try {
            when (val chairman = selectChairmanByDirectionByID(directionId)) {
                is User -> Success(chairman)
                else -> Failure(DirectionFetchingError.NO_SUCH_DIRECTION)
            }
        } catch (_: DataAccessException) {
            Failure(DirectionFetchingError.UNKNOWN_DATABASE_ERROR)
...
class FetchDeputyChairmanByDirectionID (
    private val selectDeputyChairmanByDirectionID: (Int) -> User?,
) : (Int) -> Result4k<User, DirectionFetchingError> {

    override operator fun invoke(directionId: Int): Result4k<User, DirectionFetchingError> =
        try {
            when (val deputyChairman = selectDeputyChairmanByDirectionID(directionId)) {
                is User -> Success(deputyChairman)
                else -> Failure(DirectionFetchingError.NO_SUCH_DIRECTION)
            }
        } catch (_: DataAccessException) {
            Failure(DirectionFetchingError.UNKNOWN_DATABASE_ERROR)
